...
>
> **Canary amendment (#42, ADR 0003):** while `independent_review.dispatch_mode` is `disabled_pending_e2e`, no reviewer is dispatched and the evaluator publishes a `neutral` `AI Review` check outcome — a passing conclusion for the lane. Blocking evidence still fails the check (see the threat-tier amendment below); advisory findings are recorded once as an advisory Issue mapped to the exact head (updated in place on later heads, never re-created); unreviewed auto-merge stops at `R2`. `REVIEW_REQUESTED`/`REVIEW_FALLBACK` are unreachable in this mode: a successful gate goes straight to evaluation, landing on `MERGE_PENDING` (neutral or clean) or `BLOCKED` (blocking evidence) without ever requesting a reviewer. The CI/review/conflict repair lanes post no `@codex`/`@copilot`/`@dependabot` tag either; each applies a recoverable `<lane>-dispatch-disabled` block instead that clears automatically once dispatch is enabled. Block and authority comments name the owner without an `@`-mention for the same reason. The live E2E canary drill is the removal condition for this mode (ADR 0003).
>
> **Taxonomy, threat-tier, and arming amendment (2026-08-09, ADR 0003/0004):** the orchestrator workflows are `Orchestrator: PR Lifecycle / Gate Result / Review Event / Comment Event / Watchdog`, and the advisory check is `Advisory: AI Review`. Blocking evidence is exclusively a structured verdict line `BLOCK: <CLASS> <file:line> — <exploit precondition>` (CLASS `T1-INFRA-DELETION`/`T2-BACKDOOR`/`T3-HARDCODED-SECRET`/`T4-CRITICAL-VULN`); every P0–P2 prose finding is advisory and flows into the per-PR advisory Issue (P0/P1 entries flagged prominently). Arming is evaluated, not obeyed: it requires an existing exact-head advisory evaluation with current `dispatch_policy_version` evidence and refuses only a `failure` carrying a structured verdict. Every `AI Review`/`P0/P1` reference below reads through this amendment. Block comments carry calm prose plus a per-code `Next step:` line, and a block born from a script failure quotes the underlying error.
>
> **Single-check reversal (2026-08-10, ADR 0005):** ADR 0003/0004's `Gate: Deterministic CI` gate rename (with a transitional `pr-gate-bridge` job holding the legacy `PR Gate` context green) is reverted before any repository ever flipped its ruleset. It produced exactly the checks-tab clutter it was meant to avoid: two visible entries — the renamed gate and its bridge — for what is conceptually one gate. The deterministic gate's workflow name and job name are both permanently `PR Gate`; there is no bridge job and no `required_status_context_next` transition target. `workflow_run` consumers (`Orchestrator: Gate Result`) key off the workflow name `PR Gate`. Every prior reference to `Gate: Deterministic CI` in this document is historical only.
