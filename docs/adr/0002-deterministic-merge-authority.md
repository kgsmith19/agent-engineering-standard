...
- A fine-grained PAT (`AUTOMATION_TOKEN`, Administration:read) backs the three calls `GITHUB_TOKEN` cannot perform correctly: admin settings/ruleset reads in `auto-merge.ps1`, the `gh pr ready` promotion, and arming the merge (so the merge push triggers `on: push` pipelines). Check runs stay on `GITHUB_TOKEN` for GitHub-Actions app attribution. The PAT is never exposed to PR-authored code.
