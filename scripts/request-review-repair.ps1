...
function Add-CommentOnce { param([string]$Marker,[string]$Body,$Comments) if(@($Comments|Where-Object{(Test-TrustedAutomationComment $_ ([string]$config.owner))-and[string]$_.body-like"*$Marker*"}).Count-gt 0){return}; & gh pr comment $Pr --repo $Repo --body "$Body`n`n$Marker"|Out-Host; if($LASTEXITCODE-ne 0){throw"Could not comment on $Repo PR #$Pr."} }
function Get-ActiveBlockCodes { param($Comments) $active=@{}; foreach($comment in @($Comments|Sort-Object created_at)){if(-not(Test-TrustedAutomationComment $comment ([string]$config.owner))){continue};$body=[string]$comment.body;if($body-match'<!-- automation:block:([a-z0-9-]+):[0-9a-f]{40} -->'){$active[$Matches[1]]=$true}elseif($body-match'<!-- automation:resolve:([a-z0-9-]+):[0-9a-f]{40} -->'){$active.Remove($Matches[1])}};return @($active.Keys) }
function Resolve-TransientReviewBlocks { param([string]$Head,$Comments) $active=@(Get-ActiveBlockCodes $Comments);$resolved=$false;foreach($code in @('review-request','review-fallback','review-timeout')){if($active-notcontains$code){continue};Add-CommentOnce "<!-- automation:resolve:${code}:${Head} -->" 'AUTOMATION-RECOVERED: material review evidence is now being handled by the bounded repair lane.' $Comments;$resolved=$true};if(-not$resolved){return};$fresh=@(Get-Comments);if(@(Get-ActiveBlockCodes $fresh).Count-eq 0){& gh pr edit $Pr --repo $Repo --remove-label $config.pr_automation.blocked_label 2>&1|Out-Null} }
