...
function Disable-AutoMergeIfArmed {
  $currentRaw = & gh pr view $Pr --repo $Repo --json state,autoMergeRequest 2>&1
  if ($LASTEXITCODE -ne 0) { throw "Could not re-read PR #$Pr while reconciling auto-merge." }
  $current = ($currentRaw -join "`n") | ConvertFrom-Json
  if ($current.state -eq 'OPEN' -and $null -ne $current.autoMergeRequest) {
    & gh pr merge $Pr --repo $Repo --disable-auto 2>&1 | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "Could not disable already-armed auto-merge for $Repo PR #$Pr." }
    Write-Host "AUTO-MERGE DISARMED: $Repo PR #$Pr no longer satisfies the unattended-merge contract." -ForegroundColor Yellow
  }
}

$prRaw = & gh pr view $Pr --repo $Repo --json isDraft,state,labels,baseRefName,headRefOid 2>&1
...
try {
  ...
  if ($labels -contains 'status:blocked') { throw "PR #$Pr is status:blocked." }

  $riskLabels = @($labels | Where-Object { $_ -match '^risk:R[0-4]$' })
  if ($riskLabels.Count -gt 1) { throw "PR #$Pr has multiple risk labels: $($riskLabels -join ', ')." }
  if ($riskLabels.Count -eq 1) { $Risk = $riskLabels[0].Substring(5) }
  ...
  & gh pr merge $Pr --repo $Repo --auto --squash --match-head-commit $pr.headRefOid
