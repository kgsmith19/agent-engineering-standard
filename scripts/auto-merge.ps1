...
if ($controlPlane -and [bool]$config.manual_gates.control_plane.required) {
  throw "Auto-merge refused: control-plane gate is currently justified because this PR can modify its own evaluator/authority. Removal condition: $($config.manual_gates.control_plane.removal_condition)"
}

# Fail closed unless the live GitHub integration plane is actually enforcing the intended policy.
...
if ([bool]$prRule.parameters.require_code_owner_review) { throw 'Required Code Owner review detected on the personal-repo lane.' }
...
# Require a current-head reviewer from a different provider.
